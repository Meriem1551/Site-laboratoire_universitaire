...
if (!empty($publications)) {
    ...
    $pubCard = new Card(
        ["<div class='flex items-center justify-between mb-8 pb-6 border-b border-gray-200'>
                    <div>
                        <h2 class='text-2xl font-bold text-[var(--gray-dark)]'>Publications</h2>
                        <p class='text-[var(--gray)] text-sm mt-2'>Voir ce que nos membres ont publie</p>
                    </div>
    ...
    if(!empty($members)) {
            ...
            echo "<div class='text-center py-12 bg-[var(--gray-light)] rounded-lg border-2 border-dashed border-gray-200'>";
            ...
            echo "</svg>";
            echo "<p class='text-[var(--gray)] font-medium'>Aucun membre supplémentaire</p>";
            echo "<p class='text-[var(--gray)] text-sm mt-1'>Le responsable est actuellement le seul membre de ce projet</p>";
            echo "</div>";
        }
}
    private function render_team($team){
            ...
            <div class='rounded-xl p-5 border border-blue-200 mb-6'>
                <div class='flex items-center gap-4'>
                    ...
                    <div>
                        ...
                        <div class='text-xl font-bold text-[var(--gray-dark)]'>" . $team['first_name'] . " " . $team['last_name'] . "</div>
                    </div>
                </div>
                <div>
                    <div class='text-sm font-medium text-[var(--primary)] mb-1'>". $team['speciality'] ."</div>
                    <div class='text-xl font-bold text-[var(--gray-dark)]'>" . $team['post'] . "</div>
                    <div class='text-xl font-bold text-[var(--gray-dark)]'>" . $team['grade'] . "</div>
                </div>
            </div>";
        }
        ...
        if (!empty($team['research_themes'])) {
            ...
            <div class='mb-8'>
                <div class='flex items-center gap-3 mb-4'>
                    ...
                    <h3 class='text-xl font-bold text-[var(--gray-dark)]'>Thème de recherche</h3>
                </div>
                <p class='text-[var(--gray)] text-lg leading-relaxed pl-14'>" . $team['research_themes'] . "</p>
            </div>";
        }
        ...
        if (!empty($team['description'])) {
            ...
            <div class='mb-8'>
                <div class='flex items-center gap-3 mb-4'>
                    ...
                    <h3 class='text-xl font-bold text-[var(--gray-dark)]'>Description</h3>
                </div>
                <div class='prose prose-lg max-w-none pl-14'>
                    <p class='text-[var(--gray)] leading-relaxed'>" . $team['description'] . "</p>
                </div>
            </div>";
        }
        ...
        $teamInfoCardHeader = [(new Title(
            ...
            'text-2xl font-bold text-[var(--gray-dark)] mb-8 pb-4 border-b border-gray-200',
            'h2'
        ))->render()];
        ...
        $teamInfoCard = new Card(
            ...
            [],
            'bg-[var(--white)] rounded-2xl shadow-lg border border-gray-200 p-8 hover:shadow-xl transition-shadow duration-300'
        );
...
private function render_equips($equips) {
    $equipHeader = [
        "<div class='flex items-center justify-between mb-8 pb-6 border-b border-gray-200'>
                    <div>
                        <h2 class='text-2xl font-bold text-[var(--gray-dark)]'>Équipements</h2>
                        <p class='text-[var(--gray)] text-sm mt-2'>Tous les équipements réservés par nos membres</p>
                    </div>
                    <span class='px-4 py-2 bg-blue-100 text-[var(--primary-light)] text-sm font-medium rounded-full'>
   ...
   $equipmentsHTML = '';
    foreach($equips as $equip) {
        $equipmentsHTML .= "
        <div class='bg-[var(--white)] rounded-xl border border-gray-200 p-6 hover:shadow-md transition-shadow'>
            ...
            <h3 class='text-lg font-bold text-[var(--gray-dark)] mb-2'>
                " . ($equip['name'] ?? 'Nom') . "
            </h3>
            <p class='text-[var(--gray)] text-sm'>
       ...
       echo '<section class="py-24 px-24 min-h-screen">';
            echo '<div class="container mx-auto px-4 max-w-7xl">';
                    ...
                    echo '<div class="mb-10">';
                        $title = (new Title(
                            $team['name'],
                            'text-4xl lg:text-5xl font-bold text-[var(--gray-dark)] leading-tight mb-3',
                            'h1'
                        ))->render();
                        echo $title;
                        echo "<div class='text-[var(--gray)] text-lg'>Équipe de recherche</div>";
                    echo '</div>';
    ...
    public function show_teams($teams, $allowed){
        ...
        echo '<section class="min-h-screen py-24 w-full px-12">';
            echo '<div class="mb-10">';
                echo '<h1 class="text-3xl lg:text-4xl font-bold text-[var(--gray-dark)] mb-2">Gestion des equipes</h1>';
                echo '<p class="text-[var(--gray)] text-lg">Consultez et gérez tous les equipes du système</p>';
                echo '<div class="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">';
                     $header = [
                                "<div class='text-sm text-[var(--gray)] mb-1'>Nombre d'equipe</div>"
                            ];
                            $body = [
                                "<div class='text-2xl font-bold text-[var(--gray-dark)]'>{$nb_teams}</div>"
                            ];
                    $card = new Card($header, $body, [], "border-t-4 bg-[var(--white)] border-purple-600 rounded-xl p-4 shadow-sm");        $card->render();
                echo '</div>';
                echo '<div class="bg-[var(--white)] rounded-2xl shadow-lg border border-gray-200 overflow-hidden mt-8 ">';
                    ...
                    echo '<div class="px-6 py-4 border-b border-gray-200 flex flex-col rounded-lg sm:flex-row sm:items-center sm:justify-between gap-4">';
                        echo '<h2 class="text-xl font-bold text-[var(--gray-dark)]">Liste des equipes</h2>';
                        
                        echo "<div class='flex gap-6 ml-auto'>";
                            if ($allowed['create']) {
                                echo '<a href="index.php?page=create_team" class="px-4 py-2 bg-[var(--primary)] text-[var(--white)] font-medium rounded-lg hover:bg-[var(--primary-light)] transition-colors flex items-center gap-2">';
                    ...
                    $data[] = [
                        ...
                        "Theme de recherche" => $team['research_themes'],
                         'Actions' => '<div class="relative inline-block text-left">
                                            <button type="button" class="p-2 text-gray-400 hover:text-[var(--gray)] hover:bg-gray-100 rounded-lg transition-colors action-menu-btn" data-project-id="' . $team['team_id'] . '">
                                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                                    <path d="M10 6a2 2 0 110-4 2 2 0 010 4zM10 12a2 2 0 110-4 2 2 0 010 4zM10 18a2 2 0 110-4 2 2 0 010 4z"/>
                                                </svg>
                                            </button>
                                    ' . ($allowed['update'] ? '
                                    <div class="absolute right-0 mt-2 w-56 bg-[var(--white)] rounded-lg shadow-lg border border-gray-200 z-10 hidden action-menu" data-project-id="' . $team['team_id'] . '">'.'
