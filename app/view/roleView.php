    ...
    public function show_roles($roles, $allowed){
        ...
        foreach($roles as $role) {
            $data[] = [
                ...
                'Actions' => '<div class="flex items-center gap-2 justify-end">'
                    . ($allowed['delete'] ? '<a href="index.php?page=delete_role&id=' . $role['id'] . '" 
        ...
        echo '</div>'; 
            echo "<a href='index.php?page=gestion_users' 
                class='inline-flex items-center text-[var(--primary)] hover:text-blue-800 mt-4 font-medium 
        ...
        $form->addField('name', 'Nom du rôle', 'text', '', 'Ex: Administrateur, Éditeur, etc.');
